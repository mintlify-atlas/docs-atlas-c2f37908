...
<Steps>
  ...
  <Step title="First Run">
    ...
    2. Double-click `xenia.exe` or `xenia_canary.exe`
...
<Steps>
  ...
  <Step title="Build Xenia">
    ...
    - **Debug**: `build/bin/Windows/Debug/xenia.exe`
    - **Release**: `build/bin/Windows/Release/xenia.exe`
...
xenia/
├── xenia.exe (or xenia)
├── xenia.config.toml          # Main configuration file
...
The `xenia.config.toml` file is created on first launch. Edit it to customize Xenia's behavior:
...
<Steps>
  ...
  <Step title="Replace Executable">
    Replace the old `xenia.exe` with the new one. Keep your `content/` and config files.
  </Step>
</Steps>
